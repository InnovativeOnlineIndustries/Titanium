...
public record ContentExistsCondition<T>(HolderGetter<T> registry, ResourceKey<T> contentName) implements ICondition {
    public static final MapCodec<ContentExistsCondition<?>> CODEC = RecordCodecBuilder.mapCodec(in -> in.group(
        Identifier.CODEC.fieldOf("registry").forGetter(ce -> ce.contentName().registry()),
        Identifier.CODEC.fieldOf("name").forGetter(ce -> ce.contentName().identifier()),
    ...
    public static final Identifier NAME = Identifier.fromNamespaceAndPath(Titanium.MODID, "content_exists");
