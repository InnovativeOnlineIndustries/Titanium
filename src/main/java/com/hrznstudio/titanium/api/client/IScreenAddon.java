...
public interface IScreenAddon extends IGuiEventListener {
     ...
     */
    default boolean handleClick(Screen screen, int guiX, int guiY, double mouseX, double mouseY, int mouseButton) {
        return mouseClicked(mouseX, mouseY, mouseButton);
    }

    /**
     * Called when a mouse gets dragged
     *
     * @param mouseX
     * @param mouseY
     * @return Returns true if the drag was handled
     */
    default boolean drag(int mouseX, int mouseY) {
        return false;
    }
}
