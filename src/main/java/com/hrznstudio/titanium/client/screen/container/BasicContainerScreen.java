...
public class BasicContainerScreen<T extends Container> extends ContainerScreen<T> implements IScreenAddonConsumer {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        Minecraft.getInstance().fontRenderer.drawString(name, xCenter + xSize / 2F - Minecraft.getInstance().fontRenderer.getStringWidth(name) / 2F, yCenter + 6, 0xFFFFFF);
    ...
    private void checkForMouseDrag(int mouseX, int mouseY) {
        //TODO: func_228018_at_ / mainWindow
        if (GLFW.glfwGetMouseButton(Minecraft.getInstance().func_228018_at_().getHandle(), GLFW.GLFW_MOUSE_BUTTON_LEFT) == GLFW.GLFW_PRESS) {
