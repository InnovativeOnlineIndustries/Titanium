...
public class MachineTile<T extends BasicTileBlock<T, U>, U extends MachineTile<T, U>> extends PoweredTile<T, U> implements IMachine {

    @Save
    private SidedInvHandler augmentInventory;

    public MachineTile(T blockTileBase) {
        super(blockTileBase);
        ...
        addScreenAddonFactory(getAugmentBackground());
    ...
    public ActionResultType onActivated(PlayerEntity player, Hand hand, Direction facing, double hitX, double hitY, double hitZ) {
        if (super.onActivated(player, hand, facing, hitX, hitY, hitZ) != ActionResultType.SUCCESS) {
            openGui(player);
        }
